...
- [ ] Confirmar ou criar a estrutura da skill `pdf-to-md` em `platform/skills/pdf-to-md/`, mantendo `platform/skill-runtime/` apenas como runtime canônico.
- [ ] Implementar leitura de um arquivo PDF via `PyMuPDF` sem processamento de OCR.
- [ ] Configurar para salvar o arquivo `.md` bruto concatenado em disco (`var/`).
...
- [ ] Modificar o laço de iteração de páginas no PyMuPDF para emitir e anexar uma âncora delimitadora de formato `[[Pág. N]]` a cada virada de página.
...
- [ ] Criar função de avaliação de página que calcula a proporção/quantidade de caracteres de texto útil extraídos na leitura direta.
- [ ] Definir o threshold limitante de fallback para decidir dinamicamente se a página será tratada como texto validado ou será enviada para OCR.
...
- [ ] Quando uma página for reprovada na heurística, invocar rotina de conversão dessa página do PDF para imagem isolada.
- [ ] Enviar a imagem gerada ao PaddleOCR (versão: **A definir**) para extração do texto bruto pela imagem.
- [ ] Capturar a resposta textual do OCR e concatená-la ao documento final, logo após sua respectiva âncora de página.
...
- [ ] Orquestrar o processamento híbrido (texto nativo + imagens escaneadas no mesmo PDF) com a skill.
- [ ] Verificar o armazenamento correto na pasta operante.
