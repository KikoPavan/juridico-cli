# gemini-response-schema-preflight Specification

## Purpose
Evitar que `GeminiLLMClient.generate_structured` tente qualquer chamada ao Gemini (estruturada ou fallback livre) com um schema sanitizado cuja complexidade seja conhecida por exceder a compatibilidade prática do modo `response_schema` da API, escalando diretamente para a extração por blocos com base em uma avaliação determinística e local do schema — sem depender de uma chamada real à API para descobrir a incompatibilidade.
